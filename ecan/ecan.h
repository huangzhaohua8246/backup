...
//*****************************************************************************
typedef void(*can_cb)(bool, unsigned short,unsigned short);
typedef struct {
	unsigned long cmd;
	unsigned long data;
}canmsg;
typedef enum {
	eCanSucceed,			// can口操作成功
	LoadDataSucceed,		// 数据成功载入
	eCanFail,				// 未知错误
	eCanPortOccupied,		// can口已被占用
	MsgChannelBusy,			// 邮箱通道忙
	eCanBaudInvalid,		// 波特率不支持
	eCanCallbackDuplicate,	// 多次设置回调函数
	eCanParameterInvalid,	// 无效的参数
	eCanchannelUnenough,	// 邮箱通道不足
	eCanConfigSucceed			// 初始化成功
} CanRetEnum;
typedef struct {
	// ret		- CanRetEnum
	// param1	- sdo-channel-count
	// param2	- pdo-channel-count
	// param3	- bps			1000,000/500,000
	// param4	- DBO(MC10)		1/0/NULL
	CanRetEnum (*Config)(int,int,uint32_t,int);
	// ret		- CanRetEnum
	// param1	- channelmark	1~0xFFFF
	// param2	- recvmsg-buffer
	// param3	- recvbegin
	// param4	- recvcount
	// param5	- timeout
	CanRetEnum (*RecvEcanMsg)(unsigned short, canmsg*, int, int, unsigned short);
	// ret		- CanRetEnum
	// param1	- channel	1~0xFFFF
	// param2	- sendmsg-buffer
	// param3	- sendbegin
	// param4	- sendcount
	// param5	- timeout
	CanRetEnum (*SendEcanMsg)(unsigned short, canmsg*, int, int, unsigned short);
}EcanInterface;
const EcanInterface* CreatecanInterface(int);	//CanEnum

extern volatile UINT16 cana_read;
extern volatile UINT16 current_a_read;
extern volatile BOOL is_a_r_timeout;
extern volatile UINT16 canb_read;
extern volatile UINT16 current_b_read;
extern volatile BOOL is_b_r_timeout;

extern volatile UINT16 cana_send;
extern volatile UINT16 current_a_send;
extern volatile BOOL is_a_s_timeout;
extern volatile UINT16 canb_send;
extern volatile UINT16 current_b_send;
extern volatile BOOL is_b_s_timeout;
...
typedef struct
{
		...
		tCANMsgObject MBOX8;
		tCANMsgObject MBOX9;
		tCANMsgObject MBOX10;
		tCANMsgObject MBOX11;
		tCANMsgObject MBOX12;
		tCANMsgObject MBOX13;
		tCANMsgObject MBOX14;
		tCANMsgObject MBOX15;
		tCANMsgObject MBOX16;
		tCANMsgObject MBOX17;
		tCANMsgObject MBOX18;
		tCANMsgObject MBOX19;
		tCANMsgObject MBOX20;
		tCANMsgObject MBOX21;
		tCANMsgObject MBOX22;
		tCANMsgObject MBOX23;
		tCANMsgObject MBOX24;
		tCANMsgObject MBOX25;
		tCANMsgObject MBOX26;
		tCANMsgObject MBOX27;
		tCANMsgObject MBOX28;
		tCANMsgObject MBOX29;
		tCANMsgObject MBOX30;
		tCANMsgObject MBOX31;
}
MBOXS;
typedef enum {
	Cana,

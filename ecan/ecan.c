...
#include "../../F2837xD_common/inc/hw_can.h"
#include "../../F2837xD_common/inc/hw_ints.h"
#include "../../F2837xD_common/inc/hw_memmap.h"
#include "../../F2837xD_common/inc/hw_types.h"
#include "../../F2837xD_common/driverlib/debug.h"
#include "../../F2837xD_common/driverlib/interrupt.h"
#include <stdint.h>
#include <stdbool.h>
#include "ecan.h"
#include "platform.h"
...
//*****************************************************************************
extern const EcanInterface* EcanaInterface_a(void);
extern const EcanInterface* EcanbInterface_b(void);

const EcanInterface* CreatecanInterface(int port) {
	switch (port) {
	case Cana: return EcanaInterface_a();
	case Canb: return EcanbInterface_b();
	default: return NULL;
	}
}

volatile UINT16 cana_read = 0;
volatile UINT16 current_a_read = 0;
volatile BOOL is_a_r_timeout = FALSE;
volatile UINT16 canb_read = 0;
volatile UINT16 current_b_read = 0;
volatile BOOL is_b_r_timeout = FALSE;
void OnCanRead(bool istimeout, unsigned short port, unsigned short length)
{
	if(port == Cana){
		current_a_read = length;
		cana_read += length;
		is_a_r_timeout = istimeout;
	}
	if(port == Canb){
		current_b_read = length;
		canb_read += length;
		is_b_r_timeout = istimeout;
	}
}

volatile UINT16 cana_send = 0;
volatile UINT16 current_a_send = 0;
volatile BOOL is_a_s_timeout = FALSE;
volatile UINT16 canb_send = 0;
volatile UINT16 current_b_send = 0;
volatile BOOL is_b_s_timeout = FALSE;
void OnCanSend(bool istimeout, unsigned short port, unsigned short length)
{
	if(port == Cana){
		current_a_send = length;
		cana_send += length;
		is_a_s_timeout = istimeout;
	}
	if(port == Canb){
		current_b_send = length;
		canb_send += length;
		is_b_s_timeout = istimeout;
	}
}
